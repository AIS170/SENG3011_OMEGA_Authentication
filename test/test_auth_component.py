...
def test_signup(client, test_cognito, user_data_1, clear_dynamo):
    response = client.post(
        "/signup", data=json.dumps(user_data_1), content_type="application/json"
    )
...
def test_username_in_use(client, test_cognito, user_data_1, user_data_2, clear_dynamo):
    response = client.post(
        "/signup", data=json.dumps(user_data_1), content_type="application/json"
    )
    assert response.status_code == 200
    ...
    response = client.post(
        "/signup", data=json.dumps(user_data_2), content_type="application/json"
    )
    assert response.status_code == 400
    assert response.json.get("message") == (
...
def test_admin_confirm_signup_not_allowed(
    client, test_cognito, monkeypatch, user_data_1, clear_dynamo
):
...
def test_admin_confirm_signup_bad_input(
    client, test_cognito, user_data_1, clear_dynamo
):
    response = client.post(
        "/signup", data=json.dumps(user_data_1), content_type="application/json"
    )
    ...
    response = client.post(
        "/admin/confirm_signup", data=json.dumps({}), content_type="application/json"
    )
...
def test_admin_confirm_signup_user_not_found(client, test_cognito, clear_dynamo):
    response = client.post(
        "/admin/confirm_signup",
...
def test_confirm_signup(client, test_cognito, monkeypatch, user_data_1, clear_dynamo):
    def test_confirmation(**kwargs):
        return {}

    monkeypatch.setattr(test_cognito["client"], "confirm_sign_up", test_confirmation)
...
def test_confirm_signup_bad_input(client, test_cognito, user_data_1, clear_dynamo):
    response = client.post(
        "/signup", data=json.dumps(user_data_1), content_type="application/json"
    )
    assert response.status_code == 200
...
def test_confirm_signup_invalid_code(client, test_cognito, user_data_1, clear_dynamo):
    response = client.post(
        "/signup", data=json.dumps(user_data_1), content_type="application/json"
    )
...
def test_incorrect_password(client, test_cognito, user_data_1, clear_dynamo):
    ...
    response = client.post(
        "/login",
        data=json.dumps(
            {"username": user_data_1["username"], "password": "goodPassword456!"}
        ),
        content_type="application/json",
    )
    assert response.status_code == 401
    assert response.json.get("message") == ("The username or password is incorrect.")
...
def test_logout_no_token(client, test_cognito, user_data_1, clear_dynamo):
    ...
    response = client.post(
        "/logout", headers={"Authorization": ""}, content_type="application/json"
    )
    assert response.status_code == 401
...
def test_delete_user_bad_input(client, test_cognito, user_data_1, clear_dynamo):
    response = client.post(
        "/signup", data=json.dumps(user_data_1), content_type="application/json"
    )
    assert response.status_code == 200
...
def test_delete_user_incorrect_password(
    ...
    response = client.delete(
        "/delete_user",
        data=json.dumps(
            {"username": user_data_1["username"], "password": "incorrectPassword123!"}
        ),
        content_type="application/json",
    )
    assert response.status_code == 401
...
def test_delete_unconfirmed_user(client, test_cognito, user_data_1, clear_dynamo):
    response = client.post(
        "/signup", data=json.dumps(user_data_1), content_type="application/json"
    )
    assert response.status_code == 200
...
# Cognito response is mocked as there is no way to retrieve the confirmation
# code from an email during testing
def test_forgot_password(client, test_cognito, user_data_1, clear_dynamo, monkeypatch):
    response = client.post(
        "/signup", data=json.dumps(user_data_1), content_type="application/json"
    )
    assert response.status_code == 200
    ...
    monkeypatch.setattr(test_cognito["client"], "forgot_password", mock_forgot_password)
    ...
    monkeypatch.setattr(
        test_cognito["client"], "confirm_forgot_password", mock_confirm_forgot_password
    )
...
def test_forgot_password_bad_input(client, test_cognito, user_data_1, clear_dynamo):
    response = client.post(
        "/signup", data=json.dumps(user_data_1), content_type="application/json"
    )
    assert response.status_code == 200
...
def test_forgot_password_with_invalid_user(
    client, user_data_1, test_cognito, clear_dynamo
):
    response = client.post(
        "/forgot_password",
        data=json.dumps({"username": user_data_1["username"]}),
...
def test_confirm_forgot_password_bad_input(
    client, user_data_1, test_cognito, clear_dynamo, monkeypatch
):
    response = client.post(
        "/signup", data=json.dumps(user_data_1), content_type="application/json"
    )
...
def test_update_email_bad_inputs(user_data_1, client, test_cognito, clear_dynamo):
    response = client.post(
        "/signup", data=json.dumps(user_data_1), content_type="application/json"
    )
    assert response.status_code == 200
    ...
    assert response.status_code == 400
    assert response.json.get("message") == ("All fields must be provided (new_email)")


# Test for error when email is updated with invalid token
def test_update_email_invalid_token(user_data_1, client, test_cognito, clear_dynamo):
    response = client.post(
        "/signup", data=json.dumps(user_data_1), content_type="application/json"
    )
    assert response.status_code == 200
...
def test_update_email_invalid_email(user_data_1, client, test_cognito, clear_dynamo):
    response = client.post(
        "/signup", data=json.dumps(user_data_1), content_type="application/json"
    )
    assert response.status_code == 200
...
def test_update_password_bad_input(user_data_1, client, test_cognito, clear_dynamo):
    response = client.post(
        "/signup", data=json.dumps(user_data_1), content_type="application/json"
    )
    assert response.status_code == 200
    ...
    response = client.put(
        ...
        headers={"Authorization": f"Bearer {token}"},
        data=json.dumps(
            {"old_password": user_data_1["password"], "new_password": None}
        ),
        content_type="application/json",
    )
...
def test_update_password_no_token(user_data_1, client, test_cognito, clear_dynamo):
    response = client.post(
        "/signup", data=json.dumps(user_data_1), content_type="application/json"
    )
    assert response.status_code == 200
...
def test_update_password_invalid_token(user_data_1, client, test_cognito, clear_dynamo):
    response = client.post(
        "/signup", data=json.dumps(user_data_1), content_type="application/json"
    )
    assert response.status_code == 200
...
def test_update_email_invalid_new_password(
    user_data_1, client, test_cognito, clear_dynamo
):
    response = client.post(
        "/signup", data=json.dumps(user_data_1), content_type="application/json"
    )
    ...
    response = client.put(
        ...
        headers={"Authorization": f"Bearer {token}"},
        data=json.dumps(
            {"old_password": user_data_1["password"], "new_password": "badpassword"}
        ),
        content_type="application/json",
    )

    assert response.status_code == 400
    assert response.json.get("message") == ("The new password is of invalid format")
...
def test_update_password_invalid_old_password(
    ...
    response = client.put(
        ...
        headers={"Authorization": f"Bearer {token}"},
        data=json.dumps(
            {"old_password": "goodPassword1234!", "new_password": "greatPassword123!"}
        ),
        content_type="application/json",
    )
...
def test_user_info_no_token(user_data_1, client, test_cognito, clear_dynamo):
    ...
    response = client.get(
        "/user_info", headers={"Authorization": ""}, content_type="application/json"
    )
...
def test_user_info_invalid_token(user_data_1, client, test_cognito, clear_dynamo):
    response = client.post(
        "/signup", data=json.dumps(user_data_1), content_type="application/json"
    )
    assert response.status_code == 200
